...
class TestTransformer(unittest.TestCase):
    def test_transformer(self):
        transformer = MultiContrastSwinTransformer(
            dim=16, num_layers=4, window_size=(4, 4), shift_size=(2, 2), num_contrats=4, num_heads=4)
...
class TestMultiModalityGeneration(unittest.TestCase):
    ...
    def test_model_on_cuda(self):
        if torch.cuda.is_available():
            task = MultiModalityGeneration(
                dim=4, num_layers=4, window_size=(4, 4), shift_size=(2, 2), num_contrats=4, num_heads=4).cuda()
